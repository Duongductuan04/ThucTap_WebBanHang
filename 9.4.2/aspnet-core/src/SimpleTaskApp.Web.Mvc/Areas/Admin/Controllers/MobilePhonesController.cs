...
namespace SimpleTaskApp.Areas.Admin.Controllers
{
    ...
    public async Task<IActionResult> ImportFromExcel(IFormFile file)
    {
      ...
      ImportExcelResultDto result;

      using (var stream = file.OpenReadStream())
      {
        result = await _mobilePhoneAppService.ImportFromExcelAsync(stream);
      }
      ...
      if (result.ErrorCount == 0)
      {
        ...
        {
          success = true,
          message = $"{result.SuccessCount} sản phẩm đã được import thành công."
        });
      }
      else
      {
        ...
        {
          success = false,
          message = $"{result.SuccessCount} sản phẩm thành công, {result.ErrorCount} dòng lỗi.",
          errors = result.Errors // trả về danh sách lỗi chi tiết
        });
      }
    }
