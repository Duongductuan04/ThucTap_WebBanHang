  ...
  var _$cartsTable = _$table.DataTable({
    ...
    columnDefs: [
      ...
      { targets: 1, data: 'name', title: l('Name') },
      {
        ...
        data: 'price',
        title: l('Price'),
        render: (data) => data ? data.toLocaleString('vi-VN') + ' đ' : ''
      },
      {
        targets: 3,
        data: 'discountPrice',
      ...
      // 🔹 Cột màu
      {
        ...
        render: (data, type, row) => {
          return row.colorName ? row.colorName : '-';
        }
      }
,
      // 🔹 Cột ảnh theo màu
      {
        ...
        title: l('Image'),
        render: (data, type, row) => {
          const imgUrl = row.colorImageUrl ? row.colorImageUrl : data;
          return imgUrl ? `<img src="${imgUrl}" style="width:50px; height:50px; object-fit:cover; border-radius:4px;" />` : '';
        }
      },
      {
        targets: 9,
    ...
    <div class="d-flex justify-content-around">
        <button type="button" class="btn btn-sm btn-secondary edit-cart" 
                data-id="${row.id}" 
                title="${l('Edit')}">
  ...
  $(document).on('click', '.delete-cart', function () {
    ...
    abp.message.confirm(
      ...
      function (isConfirmed) {
        if (isConfirmed) {
          _cartService.delete({ id: id }).done(function () {
