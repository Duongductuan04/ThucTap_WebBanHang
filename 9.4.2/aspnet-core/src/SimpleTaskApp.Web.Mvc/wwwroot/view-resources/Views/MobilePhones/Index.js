...
<div class="d-flex justify-content-around">
    <button type="button" class="btn btn-sm btn-info detail-mobilephone" data-id="${row.id}" 
            data-toggle="modal" data-target="#MobilePhoneDetailModal" title="${l('Detail')}">
        <i class="fas fa-eye"></i>
    </button>
    <button type="button" class="btn btn-sm btn-secondary edit-mobilephone" data-id="${row.id}" 
            data-toggle="modal" data-target="#MobilePhoneEditModal" title="${l('Edit')}">
        <i class="fas fa-edit"></i>
    </button>
    <button type="button" class="btn btn-sm btn-danger delete-mobilephone" data-id="${row.id}" data-name="${row.name}" title="${l('Delete')}">
  ...
  $('#importExcelForm').submit(function (e) {
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        // res.result mới là ImportExcelResultDto thực sự
        var result = res.result;

        // Thông báo
        var message = result.message; // controller đã tạo sẵn thông báo

        if (result.errors && result.errors.length > 0) {
          message += `<br/>Chi tiết lỗi:<ul>`;
        ...
        // Hiển thị thông báo
        abp.notify.info(message, null, { messageType: 'info', position: 'topRight' });
