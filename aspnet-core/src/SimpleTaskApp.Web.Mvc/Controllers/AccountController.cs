...
using SimpleTaskApp.Web.Views.Shared.Components.TenantChange;
using SimpleTaskApp.Web.Models.Account;
...
namespace SimpleTaskApp.Web.Controllers
{
    ...
    public class AccountController : SimpleTaskAppControllerBase
    {
        ...
        public virtual async Task<JsonResult> Login(LoginViewModel loginModel, string returnUrl = "", string returnUrlHash = "")
        {
            ...
            var targetUrl = await GetAppHomeUrl(user);
            ...
            try
            {
                ...
                if (user.IsActive && (user.IsEmailConfirmed || !isEmailConfirmationRequiredForLogin))
                {
                    ...
                    if (loginResult.Result == AbpLoginResultType.Success)
                    {
                        ...
                        var targetUrl = await GetAppHomeUrl(user); // cần user từ loginResult
        ...
        public async Task<string> GetAppHomeUrl(User user)
        {
            // Kiểm tra nếu user có role Admin
            var roles = await _userManager.GetRolesAsync(user);
            if (roles.Contains("Admin")) // hoặc tên role admin bạn dùng
            {
                return Url.Action("Index", "Home", new { area = "Admin" });
            }

            // User bình thường -> chuyển sang frontend home
        ...
        // Cập nhật NormalizeReturnUrl để async compatible
        private string NormalizeReturnUrl(string returnUrl, Func<string> defaultValueBuilder = null)
        {
            if (defaultValueBuilder == null)
            {
                defaultValueBuilder = () => Url.Action("Index", "Home", new { area = "" });
            }
